      ...
      {courses.length > 0 ? (
        <div className="mb-10 mt-10">
          {courses.map((order) => (
            <div
              key={order._id}
              className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-4 justify-center items-stretch gap-x-2 gap-y-2"
            >
              {order.purchasedCourses.map((myCourse) => (
                <Link
                  key={myCourse.courseId._id}
                  to={`/course/${myCourse.courseId._id}`}
                >
                  <div className="mb-4 hover:shadow-xl w-full flex-shrink-0">
                    <div className="p-4 shadow-md h-full flex flex-col justify-between">
                      <div className="flex flex-col justify-center items-center">
                        ...
                        />
                        <div className="px-4">
                          <div className="text-lg text-green-600 font-semibold lg:text-lg md:text-base">
        ...
        <div className="flex flex-col mt-6 justify-center items-center">
          ...
          <div className="text-xl font-serif underline underline-offset-4 p-3 text-blue-700 hover:text-2xl">
            <Link to="/find-courses">Find courses</Link>
          </div>
        </div>
      )}
    </div>
